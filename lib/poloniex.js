...
const WS2_URI = 'wss://ws.poloniex.com/ws/public';
    ...
    this.returnCurrencies()
      ...
      .then((currencyPairs) => {
        ...
        this.ws.onmessage = (e) => {
          ...
          const { channel, action, data } = msg;
          if (action && channel === 'book_lv2') {
            this.emit('orderBookMessage', { action, data });
            return;
          }
          if (channel === 'book' && data) {
            this.emit('orderBookMessage', { action: 'update', data });
            return;
          }
          ...
          switch (channelId) {
            ...
            default: {
              if (Number.isInteger(channelId) && 0 < channelId && channelId < 1000) {
                ...
                let rawDataArray = msg[2];
                if (!rawDataArray) {
                  this.emit('error', `Received empty data for channel { channelId: ${channelId}, channelName: ${channelName} }`);
                  return;
                }
                let dataArray = [];
                rawDataArray.forEach((rawData) => {
